<template>
  <form @submit.prevent="insertTable" class="form">
    <label>
      <div>{{ translations && translations.rows || "Rows" }}</div>
      <input type="number" style="width: 60px" min="2" v-model="rows">
    </label>
    <label>
      <div>{{ translations && translations.columns || "Columns" }}</div>
      <input type="number" style="width: 60px" min="2" v-model="cols">
    </label>

    <button type="submit">{{ translations && translations.insert || "Insert" }}</button>
  </form>
</template>
...
export default {
  ...
  methods: {
    insertTable() {
      const rows = (
        "<tr>" + "<td><br></td>".repeat(this.cols) + "</tr>"
      ).repeat(this.rows);
